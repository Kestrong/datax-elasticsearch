...
public class ESWriter extends Writer {
    ...
    public static class Job extends Writer.Job {
        ...
        private boolean flatToNested;

        @Override
        public void init() {
            this.conf = super.getPluginJobConf();
            flatToNested = conf.getBool("flatToNested", false);
        }
        ...
        private void genRecMapping(List<ESColumn> columnList, List column, Map<String, Object> propMap) {
            if (column != null) {
                for (Object col : column) {
                    JSONObject jo = JSONObject.parseObject(col.toString());
                    String colName = jo.getString("name");
                    String colTypeStr = jo.getString("type");
                    Integer colNo = jo.getInteger("colNo");
                    Boolean ignore = jo.getBoolean("ignore");
                    if (colTypeStr == null) {
                    ...
                    columnItem.setColNo(colNo);
                    columnItem.setIgnore(ignore == null ? Boolean.FALSE : ignore);
                    ...
                    columnList.add(columnItem);
                    JSONArray child = jo.getJSONArray("child");
                    if (flatToNested && child != null && child.size() > 0) {
                        List<ESColumn> childColumns = new ArrayList<>();
                        columnItem.setChild(childColumns);
                        Map<String, Object> o = (Map<String, Object>) propMap.computeIfAbsent(colName, k -> new HashMap<>());
                        o.put("type", colTypeStr);
                        Map<String, Object> innerField = (Map<String, Object>) o.computeIfAbsent("properties", k -> new HashMap<String, Object>());
                        genRecMapping(childColumns, child, innerField);
                    } else {
        ...
        private String genMappings(String typeName) {
            ...
            genRecMapping(columnList, column, propMap);

            String columnJson = JSON.toJSONString(columnList);
            conf.set(WRITE_COLUMNS, columnJson);
    ...
    public static class Task extends Writer.Task {
        ...
        public void init() {
            ...
            this.flatToNested = conf.getBool("flatToNested", false);
        ...
        private String getRecColumn(Map<String, Map<ESColumn, Object>> records, List<ESColumn> columns, Record record) {
            String id = null;
            for (ESColumn column : columns) {
                if (column.getType().equals(ESFieldType.ID.name().toLowerCase())) {
                    id = record.getColumn(column.getColNo()).asString();
                    break;
                }
            }
            if (id == null) {
            ...
            Map<ESColumn, Object> data = records.computeIfAbsent(id, k -> new HashMap<>());
            for (ESColumn column : columns) {
                if (column.hasChild()) {
                    Map<String, Map<ESColumn, Object>> childData = (Map<String, Map<ESColumn, Object>>) data.computeIfAbsent(column, k -> new HashMap<>());
                    getRecColumn(childData, column.getChild(), record);
                } else {
                    data.put(column, record.getColumn(column.getColNo()));
                }
            }
            return id;
        }

        public void startWriteNested(RecordReceiver recordReceiver) {
            long total = 0;
            Record record = null;
            Map<String, Map<ESColumn, Object>> records = new HashMap<>(this.batchSize, 1);
            String lastId = "***";
            while ((record = recordReceiver.getFromReader()) != null) {
                String id = getRecColumn(records, columnList, record);
                if (!id.equals(lastId) && records.keySet().size() >= this.batchSize) {
                    total += doBatchInsert(records);
                    records.clear();
                }
                lastId = id;
            }
        ...
        private String setColumnValue(ESFieldType columnType, ESColumn esColumn, Column column, Map<String, Object> data, String columnName) {
            String id = null;
            if (!columnType.equals(ESFieldType.ID) && esColumn.getIgnore()) {
                return id;
            }
            ...
            if (esColumn.isArray() != null && esColumn.isArray()) {
                ...
                if (!columnType.equals(ESFieldType.DATE)) {
                    ...
                    for (int pos = 0; pos < dataList.length; pos++) {
                        dataList[pos] = getDateStr(esColumn, column);
                    }
                    data.put(columnName, dataList);
                }
            } else {
                switch (columnType) {
                    case ID:
                        ...
                        if (!esColumn.getIgnore()) {
                            data.put(columnName, id);
                        }
                        break;
                    case DATE:
                        try {
                            String dateStr = getDateStr(esColumn, column);
                            data.put(columnName, dateStr);
                        } catch (Exception e) {
                            log.error(String.format("时间类型解析失败 [%s:%s] exception: %s", columnName, column.toString(), e.toString()));
                        }
                        break;
                    case KEYWORD:
                    case STRING:
                    ...
                    case GEO_SHAPE:
                        ...
                        break;
                    default:
                        log.error("类型错误:不支持的类型:" + columnType + " " + columnName);
                }
            }
            return id;
        }

        private void getRecData(Map<ESColumn, Object> records, Map<String, Object> data) {
            for (Map.Entry<ESColumn, Object> record : records.entrySet()) {
                ...
                if (value instanceof Column) {
                    ...
                    ESFieldType columnType = ESFieldType.getESFieldType(record.getKey().getType());
        ...
        private long doBatchInsert(final Map<String, Map<ESColumn, Object>> writerBuffer) {
            final Bulk.Builder bulkaction = new Bulk.Builder().defaultIndex(this.index).defaultType(this.type);
            for (Map.Entry<String, Map<ESColumn, Object>> entry : writerBuffer.entrySet()) {
                Map<String, Object> data = new HashMap<>();
                getRecData(entry.getValue(), data);
                if (entry.getKey().startsWith("_")) {
                    //id = UUID.randomUUID().toString();
                    bulkaction.addAction(new Index.Builder(data).build());
                } else {
                    bulkaction.addAction(new Index.Builder(data).id(entry.getKey()).build());
                }
            }
            try {
                return RetryUtil.executeWithRetry(new Callable<Integer>() {
                    ...
                    public Integer call() throws Exception {
                        ...
                        if (jestResult.isSucceeded()) {
                            return writerBuffer.keySet().size();
                        }
                        ...
                        if (esClient.isBulkResult(jestResult)) {
                            ...
                            for (BulkResult.BulkResultItem item : items) {
                                if (item.error != null && !"".equals(item.error)) {
                                    log.error(String.format("id:[%s],index:%s,status:[%d], error: %s", item.id, item.index, item.status, item.error));
                                }
                            }
                            return writerBuffer.keySet().size() - brst.getFailedItems().size();
                        } else {
        ...
        public void startWrite(RecordReceiver recordReceiver) {
            if (this.flatToNested) {
                this.startWriteNested(recordReceiver);
            } else {
                this.startWriteFlat(recordReceiver);
            }
        }

        public void startWriteFlat(RecordReceiver recordReceiver) {
